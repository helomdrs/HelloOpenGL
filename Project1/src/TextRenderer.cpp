	...
	:m_Shader(shader), m_ScreenWidth(width), m_ScreenHeight(height)
{
	...
	m_VertexBufferObject = std::make_unique<VertexBuffer>(nullptr, 4 * 4 * sizeof(float));
...
{
	int totalTextures = m_Characters.size();
...
void TextRenderer::RenderText(const std::string& text, float posX, float posY, float scale, const glm::vec3& color)
{
	...
	for (c = text.begin(); c != text.end(); c++)
	{
		...
		float xPos = posX + character.Bearing.x * scale;
		float yPos = posY + (m_Characters['H'].Bearing.y - character.Bearing.y) * scale;
...
void TextRenderer::LoadFont(const std::string& fontPath, unsigned int fontSize)
{
	...
	{
		std::cout << "ERROR::FREETYPE: Could not load the font " << fontPath << std::endl;
		return;
	}
